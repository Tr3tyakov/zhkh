...
from app.domain.common.interfaces.repositories.reference_book_value_repository import IReferenceBookValueRepository
...
class HouseDataMapper:
    def __init__(
            self,
            reference_book_value_repository: IReferenceBookValueRepository
    ):
    ...
    async def get_reference_values_map(self) -> Dict[int, str]:
        ...
        book_values = await self._reference_book_value_repository.get_reference_book_values(
    ...
    async def map_house_fields(self, house) -> Dict[str, Any]:
        ...
        for column in house.__table__.columns:
            ...
            if isinstance(val, int) and val in ref_map:
