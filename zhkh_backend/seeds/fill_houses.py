...
async def import_houses(session: AsyncSession, filepath: str):
    existing_houses = await session.execute(
        select(House.id).limit(1)
    )
    ...
    company = (
        (
            await session.execute(
                select(Company).where(Company.inn == "123456789012")
            )
        )
        .scalars()
        .first()
    ...
    for h in houses:
        ...
        house = House(
            ...
            non_residential_units_count=parse_int(main.get("Количество нежилых помещений")),
            ...
            # Энергоэффективность
            energy_efficiency_class=get_ref_id(ref_values, "Класс энергетической эффективности",
                                               main.get("Класс энергетической эффективности")),
            energy_survey_date=parse_date(main.get("Дата проведения энергетического обследования")),
            ...
            capital_repair_fund=get_ref_id(ref_values, "Формирование фонда кап. ремонта",
                                           main.get("Формирование фонда кап. ремонта")),
            parking_area=parse_float(main.get("Площадь парковки м 2")),
            has_accessibility=get_ref_id(ref_values,
            ...
            building_wear_date=parse_date(main.get("Дата, на которую установлен износ здания")),
            ...
            common_property_area=parse_float(main.get("Площадь помещений общего имущества м 2")),
            land_area=parse_float(main.get("Площадь зем. участка общего имущества м 2")),

            # Статус здания
            building_series=get_ref_id(ref_values, "Серия, тип постройки", p.get("Серия, тип постройки")),
            is_cultural_heritage=get_ref_id(ref_values, "Статус объекта культурного наследия",
            ...
            drainage_system=get_ref_id(ref_values, "Система водостоков", eng.get("Система водостоков")),
            gas_supply=get_ref_id(ref_values, "Газоснабжение", eng.get("Газоснабжение")),
            hot_water_supply=get_ref_id(ref_values, "Горячее водоснабжение", eng.get("Горячее водоснабжение")),
            fire_suppression=get_ref_id(ref_values, "Система пожаротушения", eng.get("Система пожаротушения")),
            heating=get_ref_id(ref_values, "Теплоснабжение", eng.get("Теплоснабжение")),
            cold_water_supply=get_ref_id(ref_values, "Холодное водоснабжение", eng.get("Холодное водоснабжение")),
            electricity_supply=get_ref_id(ref_values, "Электроснабжение", eng.get("Электроснабжение")),
            number_of_inputs=parse_int(eng.get("Количество вводов в дом, ед.")),

            # Конструктивные элементы
            garbage_chute_type=get_ref_id(ref_values, "Тип мусоропровода", p.get("Тип мусоропровода")),
            garbage_chute=parse_bool(constr.get("Мусоропровод")),
            load_bearing_walls=get_ref_id(ref_values, "Несущие стены", p.get("Материал несущих стен")),
            basement_area=parse_float(constr.get("Площадь подвала, кв.м")),
            foundation_type=get_ref_id(ref_values, "Тип фундамента", constr.get("Фундамент")),
            foundation_material=get_ref_id(ref_values, "Материал фундамента",
                                           detail.get("Фундамент", {}).get("Материал фундамента")),
            blind_area=parse_float(detail.get("Фундамент", {}).get("Площадь отмостки")),
            overlap_type=get_ref_id(ref_values, "Тип перекрытий", p.get("Тип перекрытий")),

            # Система ГВС
            hot_water_system_type=get_ref_id(ref_values, "Тип системы горячего водоснабжения",
            ...
            hot_water_network_material=get_ref_id(ref_values, "Материал сети",
                                                  detail.get("Cистема горячего водоснабжения", {}).get(
                                                      "Материал сети")),
            hot_water_insulation_material=get_ref_id(ref_values, "Материал теплоизоляции сети",
                                                     detail.get("Cистема горячего водоснабжения", {}).get(
                                                         "Материал теплоизоляции сети")),
            hot_water_riser_material=get_ref_id(ref_values, "Материал стояков",
                                                detail.get("Cистема горячего водоснабжения", {}).get(
                                                    "Материал стояков")),

            # Система водоотведения
            sewerage_system_type=get_ref_id(ref_values, "Тип системы водоотведения",
                                            detail.get("Система водоотведения", {}).get("Тип системы водоотведения")),
            sewerage_network_material=get_ref_id(ref_values, "Материал сети",
                                                 detail.get("Система водоотведения", {}).get("Материал сети")),

            # Система газоснабжения
            gas_system_type=get_ref_id(ref_values, "Тип системы газоснабжения",
                                       detail.get("Система газоснабжения", {}).get("Тип системы газоснабжения")),

            # Внутренние стены
            internal_walls_type=get_ref_id(ref_values, "Тип внутренних стен",
                                           detail.get("Внутренние стены", {}).get("Тип внутренних стен")),

            # Фасад
            facade_wall_type=get_ref_id(ref_values, "Тип наружных стен",
                                        detail.get("Фасад", {}).get("Тип наружных стен")),
            facade_insulation_type=get_ref_id(ref_values, "Тип наружного утепления фасада",
                                              detail.get("Фасад", {}).get("Тип наружного утепления фасада")),
            facade_finishing_material=get_ref_id(ref_values, "Материал отделки фасада",
                                                 detail.get("Фасад", {}).get("Материал отделки фасада")),
            facade_last_major_repair_year=parse_int(
                detail.get("Фасад", {}).get("Год проведения последнего капитального ремонта")),

            # Крыша
            roof_shape=get_ref_id(ref_values, "Форма крыши",
                                  detail.get("Крыша", {}).get("Форма крыши")),
            attic_insulation_layers=get_ref_id(ref_values, "Утепляющие слои чердачных перекрытий",
                                               detail.get("Крыша", {}).get("Утепляющие слои чердачных перекрытий")),
            roof_support_structure_type=get_ref_id(ref_values, "Вид несущей части",
                                                   detail.get("Крыша", {}).get("Вид несущей части")),
            roof_covering_type=get_ref_id(ref_values, "Тип кровли",
                                          detail.get("Крыша", {}).get("Тип кровли")),
            roof_last_major_repair_year=parse_int(
                detail.get("Крыша", {}).get("Год проведения последнего капитального ремонта кровли")),

            # Окна
            window_material=get_ref_id(ref_values, "Материал окон", detail.get("Окна", {}).get("Материал окон")),

            # Отопление
            heating_physical_wear=parse_percent(detail.get("Система отопления", {}).get("Физический износ")),
            heating_network_material=get_ref_id(ref_values, "Материал сети",
                                                detail.get("Система отопления", {}).get("Материал сети")),
            heating_insulation_material=get_ref_id(ref_values, "Материал теплоизоляции сети",
                                                   detail.get("Система отопления", {}).get(
                                                       "Материал теплоизоляции сети")),
            heating_riser_layout_type=get_ref_id(ref_values,
                                                 "Тип поквартирной разводки внутридомовой системы отопления",
                                                 detail.get("Стояки системы отопления", {}).get(
                                                     "Тип поквартирной разводки внутридомовой системы отопления")),
            heating_riser_material=get_ref_id(ref_values, "Материал",
                                              detail.get("Стояки системы отопления", {}).get("Материал")),
            heating_riser_valve_wear=parse_percent(
                detail.get("Запорная арматура системы отопления", {}).get("Физический износ")),
            heating_device_type=get_ref_id(ref_values, "Тип отопительных приборов",
                                           detail.get("Отопительные приборы", {}).get("Тип отопительных приборов")),
            heating_device_wear=parse_percent(detail.get("Отопительные приборы", {}).get("Физический износ")),

            # Система ХВС
            cold_water_physical_wear=parse_percent(
                detail.get("Система холодного водоснабжения", {}).get("Физический износ")),
            cold_water_network_material=get_ref_id(ref_values, "Материал сети",
                                                   detail.get("Система холодного водоснабжения", {}).get(
                                                       "Материал сети")),
            cold_water_riser_wear=parse_percent(
                detail.get("Стояки системы холодного водоснабжения", {}).get("Физический износ")),
            cold_water_riser_material=get_ref_id(ref_values, "Материал сети",
                                                 detail.get("Стояки системы холодного водоснабжения", {}).get(
                                                     "Материал сети")),
            cold_water_valve_wear=parse_percent(
                detail.get("Запорная арматура системы холодного водоснабжения", {}).get("Физический износ")),

            # Система ГВС стояки/арматура
            hot_water_riser_wear=parse_percent(
                detail.get("Стояки системы горячего водоснабжения", {}).get("Физический износ")),
            hot_water_valve_wear=parse_percent(
                detail.get("Запорная арматура системы горячего водоснабжения", {}).get("Физический износ")),
